...
(async () => {
	...
	const dumpTree = (el, indent = 0, maxDepth = 12) => {
		...
		if (el.nodeType === Node.TEXT_NODE) {
			...
			if (t)
				log(
					`${pad}#text "${t.substring(0, 150).replace(/\n/g, "\\n")}"`,
				);
			return;
		}
	...
	try {
		...
		const turnColumn = scrollContainer
			? scrollContainer.querySelector(".flex-1.flex.flex-col")
			: null;
		...
		const classified = allTurns.map((turn, i) => {
			...
			const hasArtifact = !!turn.querySelector(
