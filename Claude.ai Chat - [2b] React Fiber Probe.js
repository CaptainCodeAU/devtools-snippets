	...
	const walkFiberUp = (fiber, maxDepth = 30) => {
		...
		while (current && depth < maxDepth) {
			...
			if (
				...
				typeof current.memoizedState === "object"
			) {
	...
	try {
		...
		if (btnFiber) {
			...
			findings.forEach((f, i) => {
				if (f.type && typeof f.type === "string" && f.type.length < 10)
					return; // skip div, span etc
				log(`  [${i}] ${f.type || "(dom)"}`);
			});
			log("");
			...
			if (!found)
				log("  (no long content strings found in fiber props/state)");
		} else {
				...
				for (const f of findings) {
					...
					if (f.props) {
						...
						for (const key of [
							...
							if (typeof val === "string" && val.length > 100) {
								log(
									`  📄 [${f.type}] props.${key} (${val.length} chars)`,
								);
								log(
									`     "${val.substring(0, 150).replace(/\n/g, "\\n")}…"`,
								);
								found = true;
							}
						}
