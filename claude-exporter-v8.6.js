// =============================================================================
// Claude.ai Chat Exporter v8.6 — Self-contained, API-primary, no external deps
// =============================================================================
// Run on any https://claude.ai/chat/<id> page (console paste or bookmarklet).
//
// v8.6:
//   - First turn shows "⏱ 0" (zero point) instead of nothing; elapsed
//     times render in green (.elapsed span) so they stand out from the
//     muted timestamp. transcript.md gets the "⏱ 0" too.
//   - Sticky toolbar centre shows the current position: role icon +
//     "turn N / total" pill, updated on scroll (rAF-throttled, measured
//     against the sticky bar's bottom edge).
//
// v8.5 (batch 4, user-requested):
//   - ADVANCED settings block: file-edited, deliberately NOT persisted
//     (persistence would shadow later file edits) and not in the popup.
//     Keys: toolJsonCap, imageOriginalParams, scrollStepDelayMs,
//     scrollStuckLimit, summarySeconds, errorSeconds, dateLocale,
//     zipNameSuffix ("{ts}" → timestamp), reminderTags, and htmlDefaults
//     { width, font, turnsOpen, sectionsOpen } — the last two are new
//     capability (turn/section open state on page load).
//   - All former hardcodes now read from ADVANCED (4 date-locale sites,
//     tool cap, scroll pacing, overlay durations, ZIP name).
//   - Consolidated test suite: files/test-v8.js (created via create_file
//     so it exports and folds properly), folding the v7-era 20 tests plus
//     everything since: elapsed ladder + wiring, guessLang, script-tag
//     escaping, embedded-script syntax, version consistency, ADVANCED
//     invariants. Replaces the stale exported test-v7.js.
//
// v8.4.1 (hotfix):
//   - Per-turn elapsed time (v8.3 feature) actually computes now. The
//     calculation loop and the DOM-path rawTs capture had silently missed
//     their anchors during the v8.3 edit; formatter and display sites were
//     present but had no data. Both landed with asserts this time.
//
// v8.4 (batch 3, user-requested):
//   - Every link opens in a new tab (target=_blank rel=noopener) — Source,
//     message bodies, tool-result URLs. A local file should never navigate
//     away from itself.
//   - Toolbar is sticky (top:0, page bg, hairline) so width/font/expand
//     controls are reachable mid-scroll. Left order: Collapse all ·
//     Expand turns · Expand all · Copy ALL. Divider between the width and
//     font groups.
//   - Fresh-profile defaults: width L, font A−. Saved choices still win.
//   - ⧉ enlarged (15px turns/header, 14px subsections) with bigger hit
//     area.
//   - Header card gets its own ⧉ (top-right) copying title + lean lines +
//     the full More-details block as Markdown, identical to transcript.md.
//   - Completion-summary fold line reworded to match the header/transcript
//     ("couldn't be rebuilt into files — shown as raw edits …").
//
...
(async () => {
	...
	// ── ADVANCED settings (edit here — NOT persisted, NOT in the popup) ──
	// The file is the single source of truth for these: persisted values
	// would silently shadow later file edits, so they are deliberately
	// excluded from localStorage (only CONFIG keys persist).
	const ADVANCED = {
		// Truncation limit (chars) for tool_use/tool_result JSON dumps.
		toolJsonCap: 4000,
		// Query params stripped when probing for original (non-preview)
		// image assets.
		imageOriginalParams: ["w", "h", "q", "fm", "dpr", "fit", "preview"],
		// Scroll-harvest pacing: delay per step (ms), and how many
		// no-progress steps before giving up. Raise the delay on slow
		// machines / laggy chats.
		scrollStepDelayMs: 220,
		scrollStuckLimit: 5,
		// How long the completion / error overlay stays (seconds).
		summarySeconds: 8,
		errorSeconds: 10,
		// Locale for every human-readable date in the outputs.
		dateLocale: "en-AU",
		// Appended to the ZIP filename; "{ts}" becomes a compact
		// timestamp, e.g. "_{ts}" → title_20260709-142244.zip. "" = plain.
		zipNameSuffix: "",
		// Fresh-load defaults for the exported HTML page. width/font are
		// overridden by toolbar clicks (saved per browser); the two open
		// flags apply on every load.
		htmlDefaults: {
			width: "l", // s | m | l → 950 / 1400 / 1900 px
			font: "s", // s | m | l → 12.8 / 14 / 16 px
			turnsOpen: true, // turns expanded on load
			sectionsOpen: false, // Thinking/tool sections open on load
		},
		// System tags stripped from user messages (stripSystemReminders).
		reminderTags: [
	...
	const SYSTEM_REMINDER_TAGS = ADVANCED.reminderTags;
	...
	function fmtTs(iso) {
		if (!iso) return null;
		try {
			return new Date(iso).toLocaleString(ADVANCED.dateLocale, {
				dateStyle: "medium",
				timeStyle: "short",
			});
		} catch {
	...
	try {
		...
		if (scrollContainer && needDomHarvest) {
			...
			while (guard++ < 2000) {
				...
				await sleep(ADVANCED.scrollStepDelayMs); // [v8.5]
				...
				if (scrollContainer.scrollTop === lastTop) {
					stuck++;
					if (stuck >= ADVANCED.scrollStuckLimit) {
						console.warn(
		...
		// [v8.4.1] Per-turn elapsed time vs the previous turn. This loop was
		// MEANT to ship in v8.3 — the insert missed its anchor and no-oped
		// silently, leaving formatter + display sites with nothing to show.
		if (turns.length) turns[0].elapsed = "0"; // [v8.6] zero point
		for (let i = 1; i < turns.length; i++) {
			const a = turns[i - 1].rawTs;
			const b = turns[i].rawTs;
			if (a && b)
				turns[i].elapsed = elapsedNice(new Date(b) - new Date(a));
		}
				...
				await Promise.all(
					batch.map(async (img) => {
						...
						try {
							...
							let changed = false;
							for (const p of ADVANCED.imageOriginalParams) {
		...
		const TOOL_JSON_CAP = ADVANCED.toolJsonCap; // [v8.5]
		const capJson = (s) =>
		...
		const fmtAU = (iso) =>
			iso
				? new Date(iso).toLocaleString(ADVANCED.dateLocale, {
						dateStyle: "medium",
						timeStyle: "short",
					})
				: null;
		...
		// [v8.2] build the md meta block now that stats exist.
		{
			...
			// [v8.4] header copy source = title + lean lines + details.
			headerMd = md.slice(0, i + 1).join("\n").trim();
		}
  ...
  /* [v8.6] per-turn elapsed: green so it stands out from the muted stamp */
  ...
  /* [v8.4] sticky: reachable mid-scroll */
  ...
  .copy-md.sub { padding:2px 9px; font-size:14px; }
  ...
  /* [v8.6] current-turn indicator, centred in the sticky bar */
  ...
  .more-grid .full { grid-column:1 / -1; font-style:italic; color:var(--muted); }
</style></head><body class="w-${ADVANCED.htmlDefaults.width} f-${ADVANCED.htmlDefaults.font}">`);
			L.push(`<h1>${escHtml(chatTitle)}</h1>`);
			// [v8.1-h] Human-readable local time; ISO kept on hover.
			const exportedNice = new Date(exportTime).toLocaleString(ADVANCED.dateLocale, {
				dateStyle: "full",
				timeStyle: "short",
			});
			...
			const meta = [];
			meta.push(
				`Source: <a href="${escHtml(chatUrl)}" target="_blank" rel="noopener">${escHtml(chatUrl)}</a>`,
			);
			meta.push(
			...
			L.push(
				`<div class="meta"><button class="copy-md" data-i="header" title="Copy the header as Markdown">⧉</button>${meta.join("<br>")}`,
			);
			...
			L.push(
				`<div class="toolbar"><button data-act="collapse">Collapse all</button><button data-act="expand-turns">Expand turns</button><button data-act="expand-all">Expand all</button><button data-act="copyall">Copy ALL as Markdown</button><span class="turnpos" id="turnpos"></span><span class="wbtns"><button data-w="s" title="950px">S</button><button data-w="m" title="1400px">M</button><button data-w="l" title="1900px">L</button></span><span class="fbtns"><button data-f="s" title="12.8px">A−</button><button data-f="m" title="14px">A</button><button data-f="l" title="16px">A+</button></span></div>`,
			);
			...
			turns.forEach((turn, ti) => {
				...
				L.push(
					`<details class="turn ${cls}"${ADVANCED.htmlDefaults.turnsOpen ? " open" : ""}>`,
				);
				L.push(
					`<summary><span class="role">${icon}<span class="timestamp">${escHtml(ts)}</span>${turn.elapsed ? `<span class="elapsed">⏱ ${escHtml(turn.elapsed)}</span>` : ""}</span><button class="copy-md" data-i="${ti}" title="Copy this turn as Markdown">⧉</button></summary>`,
				);
					...
					for (const block of turn.content) {
						switch (block.type) {
							...
							case "thinking": {
								...
								L.push(
									`<details${ADVANCED.htmlDefaults.sectionsOpen ? " open" : ""}><summary><span>💭 Thinking</span><button class="copy-md sub" data-b="${bi}" title="Copy this section as Markdown">⧉</button></summary><pre><code class="language-plaintext">${escHtml(block.thinking)}</code></pre></details>`,
								);
								break;
							}
							case "tool_use": {
								...
								L.push(
									`<details${ADVANCED.htmlDefaults.sectionsOpen ? " open" : ""}><summary><span>🔧 ${escHtml(block.name || "Tool use")}${stray ? " ⚠️ (not folded into an exported file)" : ""}</span><button class="copy-md sub" data-b="${bi}" title="Copy this section as Markdown">⧉</button></summary>`,
								);
								const s =
									typeof inp === "string"
										? inp
							...
							case "tool_result": {
								...
								L.push(
									`<details${ADVANCED.htmlDefaults.sectionsOpen ? " open" : ""}><summary><span>📋 ${escHtml(resultName(block))}</span><button class="copy-md sub" data-b="${bi}" title="Copy this section as Markdown">⧉</button></summary>`,
								);
								...
								if (Array.isArray(block.content))
									block.content.forEach((item) => {
										if (item.type === "text" && item.text)
											...
											if (item.url)
												L.push(
													`<p><a href="${escHtml(item.url)}" target="_blank" rel="noopener">${escHtml(item.url)}</a></p>`,
												);
										}
									});
								else if (typeof block.content === "string")
...
(() => {
  ...
  document.querySelectorAll(".copy-md").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      copy(btn, i === "idx" ? data.idx : i === "header" ? data.header : data.turns[+i] || "");
    });
  });
  ...
  let savedW = "${ADVANCED.htmlDefaults.width}"; // [v8.5] from ADVANCED
  try { savedW = localStorage.getItem(WKEY) || savedW; } catch {}
  setW(["s","m","l"].includes(savedW) ? savedW : "l");
  ...
  let savedF = "${ADVANCED.htmlDefaults.font}"; // [v8.5]
  try { savedF = localStorage.getItem(FKEY) || savedF; } catch {}
  setF(["s","m","l"].includes(savedF) ? savedF : "s");
  // [v8.6] current-turn indicator: topmost turn under the sticky bar
  const pos = document.getElementById("turnpos");
  const allTurns = [...document.querySelectorAll("details.turn")];
  let posTick = false;
  const updatePos = () => {
    posTick = false;
    if (!pos || !allTurns.length) return;
    const barH =
      (document.querySelector(".toolbar")?.getBoundingClientRect().bottom ??
        0) + 4;
    let cur = 0;
    for (let i = 0; i < allTurns.length; i++) {
      if (allTurns[i].getBoundingClientRect().bottom > barH) {
        cur = i;
        break;
      }
      cur = i;
    }
    const who = allTurns[cur].classList.contains("user") ? "👤" : "🤖";
    pos.innerHTML = who + " turn <b>" + (cur + 1) + "</b> / " + allTurns.length;
  };
  addEventListener(
    "scroll",
    () => {
...
<\/script>`);
		...
		if (!useFolder) {
			...
			a.href = url;
			const zSuf = ADVANCED.zipNameSuffix.replace(
				"{ts}",
				exportTime.replace(/[-:]/g, "").replace("T", "-").slice(0, 15),
			);
			a.download = `${safeTitle}${zSuf}.zip`;
			...
			// [v8.2] redesigned summary: destination · integrity · files
				(() => {
					...
					const lines = [
						...
						foldFailures
							? `\n⚠️ ${foldFailures} file edit${foldFailures === 1 ? "" : "s"} couldn't be rebuilt into files — shown as raw edits in the transcript (manifest → telemetry)`
							: "",
					];
					return lines.filter(Boolean).join("\n");
				})(),
		);
		await sleep(ADVANCED.summarySeconds * 1000);
	} catch (err) {
		console.error(TAG + " Export failed:", err);
		setStatus("❌ " + err.message + "\n\nCheck console for details.");
		await sleep(ADVANCED.errorSeconds * 1000);
	} finally {
		overlay.remove();
	}
})();
