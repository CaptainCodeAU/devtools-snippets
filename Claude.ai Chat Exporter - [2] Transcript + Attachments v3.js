...
(async () => {
  ...
  try {
    ...
    for (const el of turnEls) {
      ...
      const message =
        findProp(fiber, "updatedMessage", 15) || findProp(fiber, "message", 15);
      if (!message) continue;
    ...
    for (let i = 0; i < turns.length; i++) {
      ...
      if (t.role === "user") {
        ...
        // Images in this turn
        const imgs = turnEls[i]?.querySelectorAll('img[src*="/files/"]') || [];
        for (const img of imgs) {
          const alt = img.getAttribute("alt") || "image";
          const safe = alt.replace(/[^a-zA-Z0-9_.-]/g, "_").substring(0, 80);
    ...
    const manifest = {
      ...
      images: Array.from(document.querySelectorAll('img[src*="/files/"]')).map(
        (img) => ({
