...
(async () => {
	...
	try {
		if (window.showDirectoryPicker) {
			...
			const safeFolderName = chatTitle
				...
				.substring(0, 80);
			folderHandle = await parentHandle.getDirectoryHandle(
				safeFolderName,
				{ create: true },
			);
			useFolder = true;
		...
		if (e.name === "AbortError") {
			...
			console.warn(
				"File System Access API unavailable or failed:",
				e.message,
			);
		}
		useFolder = false;
	...
	try {
		...
		const nodeToMd = (node, depth = 0) => {
			...
			if (node.getAttribute("data-testid") === "file-thumbnail")
				return "";
			...
			if (tag === "pre") {
				...
				const langClass = (codeEl?.className || "").match(
					/language-(\w+)/,
				);
				...
				const lang =
					langClass?.[1] || headerEl?.textContent?.trim() || "";
				return "\n```" + lang + "\n" + raw.trim() + "\n```\n";
			}
		...
		for (const turn of allTurns) {
			turnNum++;
			const hasUserMsg = !!turn.querySelector(
			...
			if (hasUserMsg) {
				...
				const userMsgEl = turn.querySelector(
					'[data-testid="user-message"]',
				);
				...
				for (const block of responseEl.children) {
					...
					if (toolLabel && hasGridPattern) {
						...
						md.push("");
						const markdownEl =
							block.querySelector(".standard-markdown");
						if (markdownEl) md.push(cleanMd(nodeToMd(markdownEl)));
						md.push("");
						md.push("</details>");
						md.push("");

						const artifactCell = block.querySelector(
							".artifact-block-cell",
						);
						if (artifactCell) {
					...
					const markdownEl =
						block.querySelector(".standard-markdown") || block;
			...
			if (turnNum % 10 === 0) {
				setStatus(
					`📦 Processed ${turnNum}/${allTurns.length} turns...`,
				);
			}
		}
		...
		if (uniqueBtns.length > 0) {
			if (useFolder) {
				...
				setStatus(
					`📥 Downloading ${uniqueBtns.length} artifacts to folder...`,
				);
				...
				for (let i = 0; i < uniqueBtns.length; i++) {
					...
					if (capturedDownloads.length > 0 && lastDownloadName) {
						const { blob } =
							capturedDownloads[capturedDownloads.length - 1];
						const safeName = lastDownloadName.replace(
							/[^a-zA-Z0-9_.-]/g,
							"_",
						);
						try {
							const fh = await artifactsFolder.getFileHandle(
								safeName,
								{ create: true },
							);
							const writable = await fh.createWritable();
							...
							console.warn(
								`Failed to write artifact "${safeName}":`,
								e,
							);
						}
					} else {
						console.warn(
							`Could not intercept download for artifact: ${title}`,
						);
					}

					// Clean up object URLs
					capturedDownloads.forEach((d) =>
				...
				setStatus(
					`📥 Capturing ${uniqueBtns.length} artifacts for ZIP...`,
				);
