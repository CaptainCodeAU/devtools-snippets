...
(async () => {
  ...
  for (const btn of allBtns) {
    ...
    if (pathCandidates.length > 0) {
      pathCandidates.forEach((c) => {
        log(`     [depth ${c.depth}] ${c.key} = "${c.val}" (${c.component})`);
      });
    } else {
  ...
  Object.defineProperty(window, "fetch", {
    ...
    value: function (...args) {
      const url =
        typeof args[0] === "string" ? args[0] : args[0]?.url || "(Request)";
  ...
  if (navigator.serviceWorker?.controller) {
    ...
    navigator.serviceWorker.controller.postMessage = function (msg, ...rest) {
      captured.push({
        type: "SW.postMessage",
