...
| **Claude.ai Chat - Artifact Download Probe**                | Claude.ai        | Instruments `fetch`, `XHR`, `createObjectURL`, anchor creation, and `Blob` constructor to capture exactly what happens when an artifact Download button is clicked.                                               |
| **Claude.ai Chat - React Fiber Probe**                      | Claude.ai        | Walks the React fiber tree to locate artifact content stored in React state/props, bypassing the need for network interception.                                                                                   |
| **Claude.ai Chat - Artifact API Probe**                     | Claude.ai        | Extracts artifact download parameters (`filepath`, `orgUuid`, `conversationUuid`) from React fibers and hooks `fetch`/`XHR` to capture the actual API request URL and body when a Download button is clicked.     |
| **Claude.ai Chat - Artifact URL Probe**                     | Claude.ai        | Tries multiple known API URL patterns with the extracted artifact parameters to discover which endpoint returns the actual file content.                                                                          |
| **Claude.ai Chat - Artifact File Object Content Probe**     | Claude.ai        | Dumps all properties of the `file` object found at depth 6 in the artifact React fiber, revealing available metadata fields such as content, text, or data.                                                       |
