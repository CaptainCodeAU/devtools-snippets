			...
			if (props) {
				...
				for (const [key, val] of Object.entries(props)) {
					if (
						...
						continue;
					if (
						/content|artifact|data|source|text|body|markdown|raw/i.test(
							key,
						)
					) {
				...
				if (props.message && typeof props.message === "object") {
					log(
						`--- fiber[${depth}].props.message (top-level keys) ---`,
					);
				...
				if (
					...
					typeof props.updatedMessage === "object"
				) {
					log(
						`--- fiber[${depth}].props.updatedMessage (top-level keys) ---`,
					);
					...
					for (const mk of [
						...
						if (props.updatedMessage[mk]) {
							...
							if (typeof props.updatedMessage[mk] === "string") {
								log(
									`    (string, ${props.updatedMessage[mk].length} chars)`,
								);
								log(
