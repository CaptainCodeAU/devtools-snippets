// =============================================================================
// Claude.ai Chat Exporter v8 — Self-contained, API-primary, no external deps
// =============================================================================
...
(async () => {
  ...
  .meta { color:#888; font-size:0.85em; border-bottom:1px solid #333; padding-bottom:12px; margin-bottom:12px; }
  ...
  /* [v8] collapsible turns + copy toolbar */
  .toolbar { display:flex; gap:8px; margin:0 0 20px; flex-wrap:wrap; }
  .toolbar button, .copy-md { background:transparent; border:1px solid #444; color:#8b8b96;
    padding:5px 12px; border-radius:7px; cursor:pointer; font-size:12px; font-weight:600; transition:filter .15s; }
  .toolbar button:hover, .copy-md:hover { filter:brightness(1.4); }
  details.turn { margin:24px 0; border-radius:10px; border:1px solid #333; padding:0; overflow:hidden; }
  details.turn > summary { list-style:none; cursor:pointer; padding:12px 20px; display:flex; align-items:center; gap:10px; user-select:none; }
  details.turn > summary::-webkit-details-marker { display:none; }
  details.turn > summary::before { content:"▶"; font-size:10px; color:#666; transition:transform .15s; }
  ...
  .idx { margin:24px 0; padding:16px 20px; border-radius:10px; border:1px solid #333; background:#1c1c2e; }
</style></head><body>`);
			...
			L.push(
				`<div class="toolbar"><button data-act="expand">Expand all</button><button data-act="collapse">Collapse all</button><button data-act="copyall">Copy ALL as Markdown</button></div>`,
			);
			...
			turns.forEach((turn, ti) => {
				...
				// [v8] each turn is a collapsible <details> with its own
				// Copy-as-Markdown button (data-i indexes into the embedded
				// markdown array).
				L.push(`<details class="turn ${cls}" open>`);
				L.push(
					`<summary><span class="role">${escHtml(icon)}<span class="timestamp">${escHtml(ts)}</span></span><button class="copy-md" data-i="${ti}">Copy MD</button></summary>`,
				);
			...
			if (artifactFiles.length) {
				...
				L.push(
					`<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px"><strong>Artifacts Index</strong><button class="copy-md" data-i="idx">Copy MD</button></div>`,
				);
				L.push(`<ol>`);
