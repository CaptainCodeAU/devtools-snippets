// =============================================================================
// Claude.ai Chat Exporter v7 — Self-contained, API-primary, no external deps
// =============================================================================
...
//         suppressedToolBlocks — "did the export lose anything" is now
...
(async () => {
	...
	function showConfigPopup(defaults) {
		return new Promise((resolve, reject) => {
			...
			title.textContent = "Claude Chat Exporter v7";
			...
			const doExport = async () => {
				...
				try {
					...
					console.warn("[v7] Could not persist config:", e.message);
				}
		...
		try {
			...
			if (r.ok) {
				...
				if (Array.isArray(orgs) && orgs[0]?.uuid) {
					console.log("[v7] Org id resolved via /api/organizations");
					return orgs[0].uuid;
				}
			}
		} catch (e) {
			console.warn("[v7] Org fallback failed:", e.message);
		}
		return null;
	}
	...
	async function fetchConversationData(chatId) {
		try {
			...
			if (!resp.ok) {
				console.warn(`[v7] API fetch failed: ${resp.status}`);
				return null;
			}
			return await resp.json();
		} catch (e) {
			console.warn("[v7] API fetch error:", e.message);
			return null;
		}
	}
		...
		if (leafId && arr.some((m) => m.parent_message_uuid)) {
			...
			if (byUuid.has(leafId)) {
				...
				if (path.length) {
					path.reverse();
					console.log(
						`[v7] Active branch: ${path.length}/${arr.length} messages (leaf walk)`,
					);
					return path;
				}
			}
	...
	try {
		...
		console.log("[v7] Export cancelled by user");
		return;
	}
	console.log(
		"[v7] Config:",
	...
	if (cfg.skipPreviouslyExported && chatId !== "unknown") {
		...
		if (history[chatId]) {
			...
			if (skip) {
				console.log("[v7] Skipped — previously exported");
				return;
			}
		}
	}
	...
	try {
		...
		if (useFolder) {
			...
			try {
				...
				console.log(
					`[v7] Folder existed — using ${folderName}/ instead`,
				);
			} catch {
		...
		console.log(
			`[v7] Storage: ${useFolder ? "File System Access (folder)" : "inline ZIP"}`,
		);
		...
		console.log(
			`[v7] API: ${apiData ? "ok" : "unavailable"}, messages: ${apiMessages.length}, model: ${apiModel || "unknown"}`,
		);
		...
		if (scrollContainer && needDomHarvest) {
			...
			while (guard++ < 2000) {
				...
				if (scrollContainer.scrollTop === lastTop) {
					...
					if (stuck >= 5) {
						console.warn(
							"[v7] Scroll made no progress 5×, stopping harvest early",
						);
						break;
					}
				} else {
			...
			console.log(
				`[v7] Harvested ${domByUuid.size} turn(s) from DOM after scroll`,
			);
		} else if (!scrollContainer) {
			console.warn(
				"[v7] No scroll container found — relying on API only",
			);
		} else {
			console.log(
				"[v7] Scroll harvest skipped (API ok, images off) [F24]",
			);
		}
		...
		console.log(`[v7] Source: ${source} | turns: ${turns.length}`);
		...
		if (cfg.stripSystemReminders) {
			...
			console.log(
				`[v7] Stripped reminders from ${stripped} user turn(s)`,
			);
		}
		...
		if (cfg.extractArtifacts) {
				...
				for (const block of t.content) {
					...
					if (!isArtifact) {
						...
						if (hasFileText) {
							...
							if (
								...
								console.warn(
									`[v7] str_replace fold FAILED for ${key} (old_str not found${rec ? "" : ", no prior create in this transcript"}) — exported file may be stale`,
								);
								if (rec)
									blockArtifact.set(block, {
										rec,
					...
					if (
						...
						if (oldS != null && rec.content.includes(oldS)) {
							...
							rec.foldFailures++;
							console.warn(
								`[v7] Artifact update fold FAILED for "${id}" (old_str not found) — exported file may be stale`,
							);
						}
						...
						foldFailures++; // [F7]
						console.warn(
							`[v7] Artifact update for unknown id "${id}" — no create seen in this transcript`,
						);
					}
				}
			}
			...
			if (cfg.antArtifactRegex) {
				...
				if (added) console.log(`[v7] Regex added ${added} artifact(s)`);
			}
		}
		...
		console.log(`[v7] Reconstructed ${artifactFiles.length} artifact(s)`);
					...
					batch.map(async (img) => {
						...
						for (const url of candidates) {
							try {
								...
								console.warn(
									"[v7] Image fetch error:",
									e.message,
								);
							}
						}
						console.warn(`[v7] Image failed: ${img.safeName}`);
					}),
				);
			}
		}
		...
		let suppressedToolBlocks = 0; // [F26]
			...
			if (t.role === "user") {
				...
				for (const block of t.content) {
					switch (block.type) {
						...
						case "tool_use": {
							const inp = block.input || {};
							// [F1] Suppress ONLY blocks reconstruction
							// consumed (rendered as refs below), or
							// artifact-shaped strays. bash_tool and friends
							// now render.
							if (blockArtifact.has(block)) break;
							if (isArtifactShaped(block)) {
								suppressedToolBlocks++; // [F26]
								break;
							}
							md.push(
  ...
  ul,ol { padding-left:24px; } li { margin:4px 0; }
				...
				if (turn.role === "user") {
					...
					for (const block of turn.content) {
						switch (block.type) {
							...
							case "tool_use": {
								const inp = block.input || {};
								// [F1] Same rule as the md renderer: only
								// suppress consumed / artifact-shaped blocks.
								if (
									blockArtifact.has(block) ||
									isArtifactShaped(block)
								)
									break;
								L.push(
		...
		// ── [F22] Attachment extracted text → uploads/*.txt ─────────────────
		// Makes exports self-contained when the API supplied extracted
		// content. Degrades to nothing if the field is absent.
		const uploadFiles = [];
		{
			...
			for (const t of turns) {
				for (const f of t.files || []) {
					...
					let name = base + ".txt";
					let n = 2;
					while (usedUp.has(name)) name = `${base}_${n++}.txt`;
					usedUp.add(name);
					uploadFiles.push({
						exportPath: `uploads/${name}`,
						filename: f.filename,
						content: f.extracted,
					});
				}
			...
			if (uploadFiles.length)
				console.log(
					`[v7] ${uploadFiles.length} attachment text(s) will be exported to uploads/`,
				);
		}

		// ── Manifest ────────────────────────────────────────────────────────
		const outputsList = ["transcript.md"];
		outputsList.push(...uploadFiles.map((u) => u.exportPath));
		...
		const manifest = {
			...
			// [F22] Strip the extracted text blob — the manifest lists
			// metadata, the content lives in uploads/*.txt.
			uploadedFiles: turns.flatMap((t) =>
				(t.files || []).map((f) => ({
					...
					size: f.size,
					extractedTo:
						uploadFiles.find((u) => u.filename === f.filename)
							?.exportPath || null,
				})),
			),
			outputs: outputsList,
		...
		// [F22] Extracted attachment text.
		for (const u of uploadFiles) await writeFile(u.exportPath, u.content);
		...
		console.error("[v7] Export failed:", err);
