// =============================================================================
// Claude.ai Chat Exporter v7.1 — Self-contained, API-primary, no external deps
// =============================================================================
...
(async () => {
	...
	function showConfigPopup(defaults) {
		return new Promise((resolve, reject) => {
			...
			title.textContent = "Claude Chat Exporter v7.1";
		...
		const uploadFiles = [];
		{
			...
			for (const t of turns) {
				for (const f of t.files || []) {
					...
					if (cfg.downloadUploads && f.fetchPath) {
						uploadFiles.push({
							kind: "fetch",
			...
			if (uploadFiles.length) {
				...
				for (const u of uploadFiles) {
					...
					try {
						const resp = await fetch(u.fetchPath, {
							credentials: "include",
						});
						if (!resp.ok) {
							console.warn(
								`[v7] Upload fetch failed (${resp.status}): ${u.filename}`,
							);
							continue;
						}
						await writeFile(u.exportPath, await resp.blob());
						u.saved = true;
					} catch (e) {
						console.warn(
							`[v7] Upload fetch error for ${u.filename}:`,
							e.message,
						);
					}
				}
				console.log(
					`[v7] Uploads saved: ${uploadFiles.filter((u) => u.saved).length}/${uploadFiles.length}`,
				);
			}
		}
