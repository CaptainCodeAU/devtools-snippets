// =============================================================================
// Claude.ai Chat Exporter v8.2 — Self-contained, API-primary, no external deps
// =============================================================================
...
(async () => {
	...
	try {
		...
		// [v8.2] build the md meta block now that stats exist.
		{
			...
			if (foldFailures > 0)
				lean.push(
					`> ⚠️ **${foldFailures} edit(s) shown inline — not applied to exported files**`,
				);
			const d = [];
  ...
  .wbtns { margin-left:auto; display:flex; gap:6px; }
  ...
  .meta .m-model { color:var(--mauve); font-weight:700; }
  .meta .m-you { color:var(--blue); } .meta .m-claude { color:var(--peach); }
  .meta .m-warn { color:var(--yellow); }
  details.more { background:transparent; border:none; padding:0; margin:4px 0 0; }
  details.more > summary { color:var(--muted); font-size:0.9em; font-weight:700; display:inline-flex; }
  ...
  .more-grid .full { grid-column:1 / -1; font-style:italic; color:var(--muted); }
			...
			if (foldFailures > 0)
				meta.push(
					`<span class="m-warn">⚠️ ${foldFailures} edit(s) shown inline — could not be applied to exported files</span>`,
				);
			...
			L.push(
				`<div class="toolbar"><button data-act="expand">Expand all</button><button data-act="collapse">Collapse all</button><button data-act="copyall">Copy ALL as Markdown</button><span class="wbtns"><button data-w="s" title="950px">S</button><button data-w="m" title="1400px">M</button><button data-w="l" title="1900px">L</button></span></div>`,
			);
