// =============================================================================
// Claude.ai Chat Exporter v8.1 — Self-contained, API-primary, no external deps
// =============================================================================
...
(async () => {
		...
		try {
			...
			if (r.ok) {
				...
				if (Array.isArray(orgs) && orgs[0]?.uuid) {
					console.log(
						TAG + " Org id resolved via /api/organizations",
					);
					return orgs[0].uuid;
				}
			}
	...
	let safeTitle = chatTitle.replace(/[^a-zA-Z0-9_-]/g, "_").substring(0, 60);
	...
	try {
		...
		const needDomHarvest = apiMessages.length === 0 || cfg.downloadImages;
			...
			while (guard++ < 2000) {
				...
				if (scrollContainer.scrollTop === lastTop) {
					...
					if (stuck >= 5) {
						console.warn(
							TAG +
								" Scroll made no progress 5×, stopping harvest early",
						);
						break;
					}
				} else {
		...
		if (cfg.extractArtifacts) {
			...
			if (cfg.antArtifactRegex) {
				...
				if (added)
					console.log(`${TAG} Regex added ${added} artifact(s)`);
			}
		}
		...
		const toolResultMd = (block) => {
			...
			if (block.display_content != null) {
				if (typeof block.display_content === "string") {
					...
					dump(
						capJson(JSON.stringify(block.display_content, null, 2)),
					);
				}
				has = true;
			}
  ...
  body { background:var(--bg); color:var(--text); font-family:var(--mono),system-ui,sans-serif;
    max-width:950px; margin:0 auto; padding:24px; line-height:1.7; }
  ...
  .user .role { color:var(--blue); } .assistant .role { color:var(--green); }
  ...
  p code, li code, td code { background:rgba(229,169,130,0.12); color:var(--peach); padding:2px 6px; border-radius:4px; }
  ...
  details.turn.assistant { background:rgba(175,216,155,0.05); border-left:3px solid var(--green); }
  ...
  .idx { margin:24px 0; padding:16px 20px; border-radius:10px; border:1px solid var(--line);
    border-left:3px solid var(--mauve); background:var(--surface); }
			...
			L.push(
				`<div class="meta">Source: <a href="${escHtml(chatUrl)}">${escHtml(chatUrl)}</a><br>Exported: <span title="${escHtml(exportTime)}">${escHtml(exportedNice)}</span><br>Model: ${escHtml(apiModel || "unknown")}<br>Turns: ${turns.length}</div>`,
			);
			...
			turns.forEach((turn, ti) => {
				...
				L.push(
					`<summary><span class="role">${icon}<span class="timestamp">${escHtml(ts)}</span></span><button class="copy-md" data-i="${ti}" title="Copy this turn as Markdown">⧉</button></summary>`,
				);
					...
					for (const block of turn.content) {
						switch (block.type) {
							...
							case "thinking": {
								...
								const bi = blockMds.push(thinkingMd(block)) - 1;
								L.push(
				...
				for (const f of t.files || []) {
					...
					if (cfg.downloadUploads && f.fetchPath && isWebPath) {
						uploadFiles.push({
							...
							exportPath: "uploads/" + uniqueName(f.filename),
						});
					} else if (f.extracted) {
						uploadFiles.push({
							kind: "text",
							...
							content: f.extracted,
							exportPath:
								"uploads/" + uniqueName(f.filename, ".txt"),
						});
					}
				}
			}
				...
				for (const u of uploadFiles) {
					...
					for (const url of candidates) {
						try {
							...
							if (isHtmlDoc && !/\.html?$/i.test(u.filename)) {
								console.warn(
