...
class WebPageInspector {
    ...
    allElements.forEach((el) => {
      if (!standardTags.has(el.tagName) && el.tagName.includes("-")) {
        customElements.push({
          ...
          attributes: [...el.attributes].map(
            (attr) =>
              `${attr.name}="${attr.value.substring(0, 50)}${attr.value.length > 50 ? "..." : ""}"`,
          ),
  ...
  async analyzeFrameworks() {
    ...
    const detectionPatterns = {
      ...
      "Material-UI": () =>
        window.MaterialUI || document.querySelector('[class*="Mui"]'),
      "Ant Design": () =>
        window.antd || document.querySelector('[class*="ant-"]'),
    };
    ...
    Object.entries(detectionPatterns).forEach(([name, detector]) => {
      try {
        ...
        if (detected) {
          ...
          frameworks.indicators[name] =
            typeof detected === "object"
              ? "Object detected"
              : detected.toString().substring(0, 100);
        }
      } catch (e) {
  ...
  async analyzeWebComponents() {
    ...
    allElements.forEach((el) => {
      ...
      if (el.tagName.includes("-")) {
        ...
        if (el.shadowRoot) {
          components.shadowRoots.push(
            this.analyzeShadowRoot(el, el.shadowRoot),
          );
        }
      }
    });
    ...
    components.customElements.forEach((comp) => {
      console.log(
        `  - ${comp.tagName}: ${comp.hasShadowRoot ? "Has Shadow DOM" : "No Shadow DOM"}`,
      );
    });
  ...
  analyzeShadowRoot(hostElement, shadowRoot) {
    const analysis = {
      ...
      innerHTML:
        shadowRoot.innerHTML.substring(0, 500) +
        (shadowRoot.innerHTML.length > 500 ? "..." : ""),
      styles: [],
      scripts: [],
      elements: {},
      customElements: [],
  ...
  async analyzeIframes() {
    ...
    iframes.forEach((iframe, index) => {
      const detail = {
        ...
        attributes: [...iframe.attributes].map(
          (attr) => `${attr.name}="${attr.value}"`,
        ),
        dimensions: {
    ...
    analysis.details.forEach((detail) => {
      console.log(
        `  - ${detail.src || "No src"}: ${detail.classes.join(" ")} (${detail.contentAccessible ? "Accessible" : "Cross-origin"})`,
      );
    });
  ...
  async analyzeScripts() {
    ...
    scripts.forEach((script, index) => {
      ...
      if (script.src) {
        analysis.external++;
        analysis.sources.add(
          new URL(script.src, window.location.href).hostname,
        );
      } else {
        analysis.inline++;
  ...
  async analyzeStylesheets() {
    ...
    styleElements.forEach((style, index) => {
      analysis.inline.push({
        index,
        content:
          style.textContent.substring(0, 300) +
          (style.textContent.length > 300 ? "..." : ""),
        length: style.textContent.length,
        media: style.media,
    ...
    console.log(
      "CSS Variables Found:",
      Object.keys(analysis.cssVariables).length,
    );
  ...
  analyzeLocalStorage() {
    try {
      return {
        ...
        keys: [...Array(localStorage.length)].map((_, i) =>
          localStorage.key(i),
        ),
  ...
  analyzeSessionStorage() {
    try {
      return {
        ...
        keys: [...Array(sessionStorage.length)].map((_, i) =>
  ...
  async analyzeAccessibility() {
    ...
    console.log(
      ...
      accessibility.forms.withoutLabels,
    );
    console.log("Links without text:", accessibility.links.withoutText);
    console.log(
      "ARIA attributes used:",
      Object.keys(accessibility.aria.attributes).length,
    );
  ...
  checkARIAUsage() {
    ...
    return {
      ...
      roles,
      totalElements: Object.values(ariaAttributes).reduce(
        (sum, count) => sum + count,
        0,
      ),
    };
  ...
  async analyzeDynamicContent() {
    ...
    console.log(
      "Elements with loading attribute:",
      dynamic.lazyLoading.withLoadingAttr,
    );
  ...
  checkLazyLoading() {
    ...
    const lazyIframes = [
      ...document.querySelectorAll('iframe[loading="lazy"]'),
    ];
  ...
  checkDynamicImports() {
    ...
    scripts.forEach((script) => {
      if (
        ...
        script.textContent.includes("loadash")
      ) {
        dynamicImportSigns++;
      }
    });
  ...
  getGlobalEventListeners() {
    ...
    ["click", "scroll", "resize", "load", "DOMContentLoaded"].forEach(
      (event) => {
  ...
  checkCustomEvents() {
    ...
    scripts.forEach((script) => {
      if (
        script.textContent.includes("CustomEvent") ||
  ...
  determinePageType() {
    ...
    if (webComponents.customElements.length > 0)
      return "Web Components Application";
    ...
    if (structure.customElements.length > 0)
      return "Custom Elements Application";
  ...
  assessComplexity() {
    const { structure, scripts, webComponents, shadowDOM } =
  ...
  identifyIssues() {
    ...
    if (accessibility.forms.withoutLabels > 0) {
      issues.push(
        `${accessibility.forms.withoutLabels} form inputs without labels`,
      );
    }
  ...
  identifyOpportunities() {
    const opportunities = [];
    const { dynamicContent, performance, accessibility } =
      this.results.analysis;
    ...
    if (
      performance.resources.filter((r) => r.cached).length <
      performance.resources.length * 0.5
    ) {
  ...
  getDevelopmentRecommendations() {
    ...
    if (
      webComponents.customElements.length > 0 &&
      shadowDOM.shadowRoots.length === 0
    ) {
    ...
    if (frameworks.detected.length < 2) {
      recs.push(
        "Consider modern framework adoption for better maintainability",
      );
    }
...
<body>
    ...
    <div class="section">
        ...
        <div class="grid">
            ${Object.entries(this.results.recommendations || {})
              .map(
                ([category, recs]) => `
                <div class="subsection">
  ...
  analyzeComponent: (selector) => {
    ...
    return {
      ...
      shadowDOM: element.shadowRoot
        ? inspector.analyzeShadowRoot(element, element.shadowRoot)
        : null,
